...
from artifici_lda.logic.stemmer import Stemmer, PORTUGUESE, ENGLISH
from artifici_lda.logic.lda import LDA
from artifici_lda.logic.Tfidf_count_vectorizer import TfidfVectorizer

from sklearn.pipeline import Pipeline

LDA_PIPELINE_PARAMS_WORDS = {
    'stopwords__stopwords': None,
    'stemmer__language': PORTUGUESE,  # ENGLISH
...
def train_lda_pipeline_default(comments, n_topics=2, language=ENGLISH, stopwords=None):
    """
...
def train_lda_pipeline_on_words(comments, n_topics=2, language=ENGLISH, stopwords=None):
    """
    Train an LDA and transform the comments.

    :param comments: a list of strings
        call to this method failed to extract word features from the TfidfVectorizer.
    :param n_topics: the number of clusters (categories, groups, or topics) to find.
    :param language: the language, refer to snowball lemmatizer's documentation for a list
        of languages. Example: 'french', 'english'. http://snowball.tartarus.org/texts/stemmersoverview.html
